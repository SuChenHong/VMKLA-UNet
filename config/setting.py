...
class train_setting:
    ...
    model_config = {
        ...
        'drop_path_rate': 0.2,
        'load_ckpt_path': None,
        'encoder_ckpt_path': './pretrain_weights/vmamba_small_e238_ema.pth',
        'decoder_ckpt_path': None,
        'with_fusion': False, 
        'with_mae': False
    }

    polyp_datasets = ['CVC-ClinicDB', 'Kvasir', 'CVC-ColonDB', 'ETIS-LaribPolypDB']
    datasets = 'isic18' 
    if datasets == 'isic18':
        data_path = './datasets/isic18/'
    elif datasets == 'isic17':
        data_path = './datasets/isic17/'
    elif datasets == 'polyp':
        data_path = './datasets/polyp/'
    elif datasets == 'ph2':
    ...
    test_transformer = transforms.Compose([
        Normalize(datasets, train=False),
        myToTensor(),
        myResize(input_size_h, input_size_w)
    ])
    ...
    if opt == 'Adadelta':
        ...
        lr = 0.0001
        betas = (0.9, 0.999) # default: (0.9, 0.999) – coefficients used for computing running averages of gradient and its square
        eps = 1e-5 # default: 1e-8 – term added to the denominator to improve numerical stability
        # eps = 1e-8
