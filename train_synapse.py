...
from config.setting_synapse import setting_config
...
def main(config: setting_config):

    # print('#----------Creating logger----------#')
    sys.path.append(config.work_dir + '/')
    log_dir = os.path.join(config.work_dir, 'log')
    checkpoint_dir = os.path.join(config.work_dir, 'ckpts')
    # checkpoint_dir = os.path.join(pth, 'ckpts')
    ...
    if os.path.exists(os.path.join(checkpoint_dir, 'best.pth')):
        ...
        best_weight = torch.load(pth + '/ckpts/best.pth', map_location=torch.device('cpu'))
